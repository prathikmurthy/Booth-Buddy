...
import { Button, Drawer, TextInput, Textarea, Switch, ColorInput } from "@mantine/core";
...
export default function App() {
  ...
  const {data, error, mutate}: {data: Data, error: any, mutate: any} = useSWR("/api/GetRooms/" + user?.sub, fetcher);
  ...
  return (
    <>
      <div className="container mx-auto py-4">
        ...
        <Drawer opened={opened} onClose={() => setOpened(false)} title="Create a new Booth" padding={"xl"} size="xl" position="right" overlayOpacity={0.55} 
          overlayBlur={3} styles={{ drawer: { backgroundColor: "#121212"}, title: { color: "white", fontSize: "24px", }}}>
          <TextInput
            placeholder=""
            label="Booth Name"
            withAsterisk
          ...
          /> */}
          <br />
          <Button fullWidth leftIcon={<GoCloudUpload />} onClick={() => pushNewBooth()}>
            Create Booth
          </Button>
        </Drawer>
        ...
        <div className="text-white text-center flex w-full flex-col mt-44">
          <h1 className="text-5xl font-extrabold text-slate-200">Hi, <span className="text-main-accent">{user?.given_name}</span>!</h1>
